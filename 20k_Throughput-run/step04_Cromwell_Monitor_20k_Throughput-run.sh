...
count=$((`cat cromwell_diff | grep  + | grep status | grep Running | wc -l`/5))
finish=$((`cat cromwell_diff | grep + | grep status | grep Succeeded | wc -l`/5))
failed=$((`cat cromwell_diff | grep + | grep status | grep Failed| wc -l`/5))
